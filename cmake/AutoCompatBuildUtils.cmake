    ...
    elseif ("$ENV{CI_COMPILER}" MATCHES [[^clang(.*)$]])
        ...
        if (DEFINED ENV{CI_RTLIB})
            if ("$ENV{CI_RTLIB}" STREQUAL libgcc)
                list(APPEND extra_cflags -rtlib=libgcc -unwindlib=libgcc)
                list(APPEND extra_cxxflags -rtlib=libgcc -unwindlib=libgcc)
            elseif("$ENV{CI_RTLIB}" STREQUAL compiler-rt)
                list(APPEND extra_cflags -rtlib=compiler-rt -unwindlib=libunwind)
                list(APPEND extra_cxxflags -rtlib=compiler-rt -unwindlib=libunwind)
            else()
                message(WARNING "Ignoring CI_RTLIB=$ENV{CI_RTLIB}")
            endif()
        endif()
